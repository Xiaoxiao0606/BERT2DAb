...
class AntibodyFunctionPredictDatasets(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if self.args.anti == 'Developability':
            ...
            if self.labeled:
                ...
                return {'H_ids':H_ids, 'H_attention_mask':H_attention_mask, 'L_ids':L_ids,'L_attention_mask':L_attention_mask,'label':labels,'epitope_feature': 999,'H_region':H_region,'L_region':L_region}
            else:
                return {'H_ids':H_ids, 'H_attention_mask':H_attention_mask, 'L_ids':L_ids,'L_attention_mask':L_attention_mask,'epitope_feature': 999,'H_region':H_region,'L_region':L_region}
        else:
            row = self.df.iloc[index]
            H_ids,H_attention_mask,L_ids,L_attention_mask= self.get_token_ids(row)
