...
class AntibodyFunctionPredicttrainer(object):
    ...
    def iteration(self, epoch, data_loader,label, train=True):
        ...
        for i, data in data_iter:
            ...
            if self.Nb_or_Ab == 'Ab':
                outputs_logits = self.model(H_ids = data["H_ids"], H_attention_mask = data["H_attention_mask"],L_ids = data['L_ids'],L_attention_mask = data['L_attention_mask'],epitope_feature = data['epitope_feature'],H_region=data['H_region'],L_region=data['L_region'])
            else:
                outputs_logits = self.model(H_ids = data["H_ids"], H_attention_mask = data["H_attention_mask"],epitope_feature = data['epitope_feature'])
            
            predicts = nn.Softmax(dim=-1)(outputs_logits)
            

            # 2. loss
            
            loss = self.loss(outputs_logits, data['label']).cuda()
            ...
            if label == 'test':
                avg_loss += loss.item()
                correct = predicts.argmax(dim=-1).eq(data['label']).sum().item()
            ...
            if (label == 'validation') & (self.local_rank == 0):
                batch_acc = metrics.accuracy_score(gather_label.cpu(),gather_predicts.argmax(dim=-1).cpu())
                batch_F1Score = metrics.f1_score(gather_label.cpu(),gather_predicts.argmax(dim=-1).cpu(),zero_division=0)
                batch_precision = metrics.precision_score(gather_label.cpu(),gather_predicts.argmax(dim=-1).cpu(),zero_division=0)
                batch_recall = metrics.recall_score(gather_label.cpu(),gather_predicts.argmax(dim=-1).cpu(),zero_division=0)
            elif (label == 'test') & (self.local_rank == 0):
                batch_acc = metrics.accuracy_score(data['label'].cpu(),predicts.argmax(dim=-1).cpu())
                batch_F1Score = metrics.f1_score(data['label'].cpu(),predicts.argmax(dim=-1).cpu(),zero_division=0)
                batch_precision = metrics.precision_score(data['label'].cpu(),predicts.argmax(dim=-1).cpu(),zero_division=0)
                batch_recall = metrics.recall_score(data['label'].cpu(),predicts.argmax(dim=-1).cpu(),zero_division=0)
            ...
            if (label == 'test') & (self.local_rank == 0):
                pre_list_test.extend(predicts.argmax(dim=-1).cpu().numpy().tolist())
                label_list_test.extend(data['label'].cpu().numpy().tolist())
            elif (label == 'validation') & (self.local_rank == 0):
                pre_list_val.extend(gather_predicts.argmax(dim=-1).cpu().numpy().tolist())
                label_list_val.extend(gather_label.cpu().numpy().tolist())
            elif (label == 'train') & (self.local_rank == 0):
                pre_list_train.extend(gather_predicts.argmax(dim=-1).cpu().numpy().tolist())
